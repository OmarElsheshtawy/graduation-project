    ...
    <div className="page-container">
      ...
      <section style={{ padding: '3rem 0' }}>
        <div className="container" style={{ maxWidth: '760px' }}>
          ...
          {/* Profile Card */}
          <div style={{ background: 'white', borderRadius: '16px', padding: '2rem', boxShadow: '0 4px 20px rgba(0,0,0,0.08)', marginBottom: '2rem', display: 'flex', alignItems: 'center', gap: '1.5rem', flexWrap: 'wrap' }}>
            ...
            <div>
              ...
              <p style={{ margin: '4px 0', color: '#5A6C7D' }}>{user?.email}</p>
            ...
            <div style={{ marginLeft: 'auto', color: '#5A6C7D', fontSize: '0.85rem' }}>
              Member since {new Date(user?.created_at).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}
            </div>
          </div>

          {/* Tabs */}
          <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.5rem', borderBottom: '2px solid #E8EDF2' }}>
            {[['profile', '👤 Edit Profile'], ['password', '🔒 Change Password']].map(([tab, label]) => (
              <button key={tab} onClick={() => setActiveTab(tab)} style={{ padding: '0.75rem 1.5rem', background: 'none', border: 'none', borderBottom: activeTab === tab ? '3px solid #4A90E2' : '3px solid transparent', color: activeTab === tab ? '#4A90E2' : '#5A6C7D', fontWeight: 600, cursor: 'pointer', fontSize: '1rem', marginBottom: '-2px' }}>
                {label}
              </button>
            ))}
          </div>
              ...
              <>
                ...
                <form onSubmit={handleProfileSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                  ...
                  <div className="form-group">
                    ...
                    <input type="email" value={user?.email} disabled style={{ background: '#F8FAFC', color: '#5A6C7D', cursor: 'not-allowed' }} />
