...
export default function CoursesHub() {
  ...
  const borderFm  = dark ? '#374151' : '#E5E7EB'  // form / filter borders
  ...
  const filtered = COURSES
    ...
    .sort((a, b) => {
      if (sortBy === 'progress') return getCompletionPercent(b.id) - getCompletionPercent(a.id)
      if (sortBy === 'xp')       return (progresses[b.id]?.xp || 0) - (progresses[a.id]?.xp || 0)
      return 0
    })
    ...
    <div style={{ minHeight: '100vh', background: bgAlt }}>
      ...
      <div style={{ background: bg, borderBottom: `1px solid ${borderFm}`, padding: '14px 0', position: 'sticky', top: 0, zIndex: 100, boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
        <div className="container" style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
          {/* Search */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: bgAlt, border: `1.5px solid ${borderFm}`, borderRadius: 10, padding: '9px 14px', flex: 1, minWidth: 200 }}>
            <svg width="15" height="15" fill="none" stroke={textFaint} strokeWidth="2" viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/></svg>
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search courses..."
              style={{ border: 'none', outline: 'none', background: 'none', fontSize: '0.875rem', fontFamily: 'inherit', color: textBody, flex: 1 }} />
            {search && <button onClick={() => setSearch('')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: textFaint, fontSize: '1rem' }}>✕</button>}
          </div>
          {/* Level filter */}
          <select value={levelFilter} onChange={e => setLevelFilter(e.target.value)}
            style={{ padding: '9px 14px', border: `1.5px solid ${borderFm}`, borderRadius: 10, fontSize: '0.875rem', fontFamily: 'inherit', background: bg, color: textBody, cursor: 'pointer', outline: 'none' }}>
      ...
      <div className="container" style={{ padding: '36px 24px 60px' }}>
        {filtered.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '4rem', color: textFaint }}>
            ...
            <h3 style={{ fontFamily: 'var(--font-display,sans-serif)', color: textBody, marginBottom: 8 }}>No courses found</h3>
            <p>Try a different search or filter.</p>
          </div>
        ) : (
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <h2 style={{ fontFamily: 'var(--font-display,sans-serif)', fontSize: '1.1rem', color: text, margin: 0 }}>
              ...
              {filtered.map(course => {
                ...
                return (
                  <div key={course.id} onClick={() => navigate(`/learn/${course.id}`)}
                    style={{ background: cardBg, borderRadius: 20, overflow: 'hidden', border: `2px solid ${started ? course.color + '30' : border}`, cursor: 'pointer', transition: 'all 0.25s', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}
                    ...
                    onMouseLeave={e => { e.currentTarget.style.transform = 'none'; e.currentTarget.style.boxShadow = '0 2px 8px rgba(0,0,0,0.05)'; e.currentTarget.style.borderColor = started ? course.color + '30' : border }}>
                    ...
                    <div style={{ padding: '20px 20px 18px' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 14 }}>
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3, flexWrap: 'wrap' }}>
                            <h3 style={{ fontFamily: 'var(--font-display,sans-serif)', fontWeight: 700, color: text, fontSize: '0.95rem', margin: 0 }}>{course.title}</h3>
                      ...
                      {/* Description */}
                      <p style={{ fontSize: '0.82rem', color: textMuted, lineHeight: 1.6, marginBottom: 14, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                        {course.description}
                      </p>
                      ...
                      <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginBottom: 14 }}>
                        {course.units.map(u => (
                          <span key={u.id} style={{ background: bgAlt, border: `1px solid ${borderFm}`, borderRadius: 6, padding: '2px 8px', fontSize: '0.68rem', color: textMuted, display: 'flex', alignItems: 'center', gap: 3 }}>
                            {u.icon} {u.title}
                          </span>
                        ))}
                      </div>
                      ...
                      {started ? (
                        <div style={{ marginBottom: 14 }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.7rem', color: textFaint, marginBottom: 5 }}>
                          ...
                          <div style={{ height: 6, background: trackBg, borderRadius: 10, overflow: 'hidden' }}>
                            <div style={{ height: '100%', background: percent === 100 ? '#22C55E' : course.color, width: `${percent}%`, borderRadius: 10, transition: 'width 0.6s ease' }} />
                          </div>
                        </div>
                      ) : (
                        <div style={{ marginBottom: 14, fontSize: '0.72rem', color: textFaint }}>
                          {total} lessons · {course.units.length} units · {course.xpPerLesson} XP/lesson
                        </div>
                      )}

                      {/* CTA Button */}
                      <button style={{ width: '100%', padding: '10px', borderRadius: 12, background: started ? course.color : bg, color: started ? 'white' : course.color, border: `2px solid ${course.color}`, fontWeight: 700, fontSize: '0.85rem', cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.2s' }}>
